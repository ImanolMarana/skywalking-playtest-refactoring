...
public class TextParser implements Parser {
    ...
    if (line.charAt(0) == '#') {
      return parseCommentLine(line, ctx);
    }
    return parseSampleLine(line, ctx);
  }
  
  private boolean parseCommentLine(String line, Context ctx) {
    String[] parts = line.split("[ \t]+", 4);
    if (parts.length < 3) {
      return false;
    }
    if (parts[1].equals("HELP")) {
      return parseHelpComment(parts, ctx);
    } else if (parts[1].equals("TYPE")) {
      return parseTypeComment(parts, ctx);
    }
    return false;
  }
  
  private boolean parseHelpComment(String[] parts, Context ctx) {
    if (!parts[2].equals(ctx.name)) {
      if (!ctx.name.isEmpty()) {
        this.lastLineReadFromStream = parts[0];
        return true;
      }
      ctx.clear();
      ctx.name = parts[2];
  ...
  private boolean parseTypeComment(String[] parts, Context ctx) {
    if (!parts[2].equals(ctx.name)) {
      if (!ctx.name.isEmpty()) {
        this.lastLineReadFromStream = parts[0];
        return true;
  ...
  private boolean parseSampleLine(String line, Context ctx) {
