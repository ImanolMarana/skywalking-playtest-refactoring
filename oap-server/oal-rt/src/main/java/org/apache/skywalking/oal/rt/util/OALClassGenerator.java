...
public class OALClassGenerator {
    ...
    private Class generateMetricsClass(AnalysisResult metricsStmt) throws OALCompileException {
        ...
        addEmptyConstructor(metricsClass, className);
        addSourceFields(metricsStmt, metricsClass, constPool);
        generateMetricsClassMethods(metricsStmt, metricsClass, className);
        addStreamAnnotation(metricsStmt, metricsClass, constPool);

        return loadMetricsClass(metricsClass, className);
    }

    private void addEmptyConstructor(CtClass metricsClass, String className) throws OALCompileException {
        try {
    ...
    private void addSourceFields(AnalysisResult metricsStmt, CtClass metricsClass, ConstPool constPool) throws OALCompileException {
        for (SourceColumn field : metricsStmt.getFieldsFromSource()) {
            try {
                ...
                addColumnNameAnnotation(field, constPool, annotationsAttribute);
                addSeriesIdAnnotation(field, constPool, annotationsAttribute);
                addTopNAggregationAnnotation(field, constPool, annotationsAttribute);

                newField.getFieldInfo().addAttribute(annotationsAttribute);
            } catch (CannotCompileException e) {
                log.error(
                        "Can't add field(including set/get) " + field.getFieldName() + " in " + metricsClass.getName() + ".", e);
                throw new OALCompileException(e.getMessage(), e);
            }
        }
    }

    private void addColumnNameAnnotation(SourceColumn field, ConstPool constPool, AnnotationsAttribute annotationsAttribute) {
    ...
    private void addSeriesIdAnnotation(SourceColumn field, ConstPool constPool, AnnotationsAttribute annotationsAttribute) {
        if (field.isID()) {
    ...
    private void addTopNAggregationAnnotation(SourceColumn field, ConstPool constPool, AnnotationsAttribute annotationsAttribute) {
    ...
    private void generateMetricsClassMethods(AnalysisResult metricsStmt, CtClass metricsClass, String className) throws OALCompileException {
    ...
    private void addStreamAnnotation(AnalysisResult metricsStmt, CtClass metricsClass, ConstPool constPool) {
        AnnotationsAttribute annotationsAttribute = new AnnotationsAttribute(
                constPool, AnnotationsAttribute.visibleTag);
        ...
        metricsClass.getClassFile().addAttribute(annotationsAttribute);
    }

    private Class loadMetricsClass(CtClass metricsClass, String className) throws OALCompileException {
        Class targetClass;
        try {
