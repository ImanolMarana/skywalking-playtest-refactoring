...
public class GRPCMetricsExporter extends MetricFormatter implements MetricValuesExportService, IConsumer<ExportData> {
    ...
    public void consume(List<ExportData> data) {
        if (CollectionUtils.isNotEmpty(data)) {
          ...
          data.forEach(row -> exportMetric(streamObserver, exportNum, row));
    
          streamObserver.onCompleted();
    
          waitForConfirmation(status, exportNum);
    
          log.debug(
              "Exported {} metrics to {}:{} in {} milliseconds.", exportNum.get(), setting.getGRPCTargetHost(),
              setting
                  .getGRPCTargetPort(), status.getWaitTime()
          );
        }
        fetchSubscriptionList();
      }
    
      private void exportMetric(StreamObserver<ExportMetricValue> streamObserver, AtomicInteger exportNum, ExportData row) {
        ExportMetricValue.Builder builder = buildExportMetricValue(row);
        if (builder == null) {
          return;
        }
      ...
      private ExportMetricValue.Builder buildExportMetricValue(ExportData row) {
      ...
      private void waitForConfirmation(GRPCStreamStatus status, AtomicInteger exportNum) {
        long sleepTime = 0;
        long cycle = 100L;
