...
public abstract class BaseMetrics<T extends SimpleCollector, C> {
    ...
    protected C getMetric() {
        if (metricsInstance == null) {
            if (isIDReady()) {
                ...
                try {
                    if (metricsInstance == null) {
                        metricsInstance = createMetricInstance();
                    }
                } finally {
                    lock.unlock();
                }
    ...
    private C createMetricInstance() {
        String[] labelNames = createLabelNames();
        String[] labelValues = createLabelValues();
        ...
        return (C) metrics.labels(labelValues);
    }

    private String[] createLabelNames() {
    ...
    private String[] createLabelValues() {
