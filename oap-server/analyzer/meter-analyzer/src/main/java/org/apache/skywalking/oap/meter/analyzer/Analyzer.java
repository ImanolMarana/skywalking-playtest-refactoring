...
public class Analyzer {
    ...
    public void analyse(final ImmutableMap<String, SampleFamily> sampleFamilies) {
        Map<String, SampleFamily> input = filterInputSamples(sampleFamilies);
        if (input.size() < 1) {
        ...
        meterSamples.forEach((meterEntity, ss) -> {
          ...
          processMeterSamples(meterEntity, ss);
        });
      }

      private Map<String, SampleFamily> filterInputSamples(ImmutableMap<String, SampleFamily> sampleFamilies) {
      ...
      private void processMeterSamples(MeterEntity meterEntity, Sample[] ss) {
        switch (metricType) {
          case single:
            processSingleValueMetric(meterEntity, ss[0]);
            break;
          case labeled:
            processLabeledMetric(meterEntity, ss);
            break;
          case histogram:
          case histogramPercentile:
            processHistogramMetric(meterEntity, ss);
            break;
        }
      }
      
      private void processSingleValueMetric(MeterEntity meterEntity, Sample sample) {
        AcceptableValue<Long> sv = meterSystem.buildMetrics(metricName, Long.class);
        sv.accept(meterEntity, getValue(sample));
        send(sv, sample.getTimestamp());
      }
      
      private void processLabeledMetric(MeterEntity meterEntity, Sample[] ss) {
      ...
      private void processHistogramMetric(MeterEntity meterEntity, Sample[] ss) {
