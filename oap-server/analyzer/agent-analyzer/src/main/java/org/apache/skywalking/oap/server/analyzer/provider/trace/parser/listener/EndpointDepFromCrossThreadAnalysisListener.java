...
public class EndpointDepFromCrossThreadAnalysisListener extends CommonAnalysisListener implements ExitAnalysisListener, LocalAnalysisListener {
    ...
    private void parseRefForEndpointDependency(final SpanObject span, final SegmentObject segmentObject) {
      ...
      if (span.getRefsCount() > 0) {
        for (int i = 0; i < span.getRefsCount(); i++) {
          ...
          parseReferenceForDependency(span, segmentObject, reference);
        }
      }
    }

    private void parseReferenceForDependency(SpanObject span, SegmentObject segmentObject, SegmentReference reference) {
    ...
    private void handleMQorGatewaySource(RPCTrafficSourceBuilder sourceBuilder, SegmentReference reference, String networkAddressUsedAtPeer, boolean isMQ) {
