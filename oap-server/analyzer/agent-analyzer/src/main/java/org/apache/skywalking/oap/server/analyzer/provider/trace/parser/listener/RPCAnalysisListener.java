...
public class RPCAnalysisListener extends CommonAnalysisListener implements EntryAnalysisListener, ExitAnalysisListener, LocalAnalysisListener {
    ...
    if (span.getRefsCount() > 0) {
      parseEntryWithReference(span, segmentObject);
    } else if (span.getSpanLayer() == SpanLayer.MQ && StringUtil.isNotBlank(span.getPeer())) {
      parseEntryForMQWithoutReference(span, segmentObject);
    } else {
      parseEntryWithoutReference(span, segmentObject);
    }

    parseLogicEndpoints(span, segmentObject);
  }

  private void parseEntryWithReference(SpanObject span, SegmentObject segmentObject) {
    for (int i = 0; i < span.getRefsCount(); i++) {
      ...
      if (isMQ || config.getUninstrumentedGatewaysConfig()
        ...
        handleMQOrGatewaySource(sourceBuilder, reference, networkAddressUsedAtPeer, isMQ);
      } else {
        handleGeneralSource(sourceBuilder, reference);
      }
  ...
  private void handleMQOrGatewaySource(RPCTrafficSourceBuilder sourceBuilder, 
  ...
  private void handleGeneralSource(RPCTrafficSourceBuilder sourceBuilder, SegmentReference reference) {
  ...
  private void parseEntryForMQWithoutReference(SpanObject span, SegmentObject segmentObject) {
    RPCTrafficSourceBuilder sourceBuilder = new RPCTrafficSourceBuilder(namingControl);
  ...
  private void parseEntryWithoutReference(SpanObject span, SegmentObject segmentObject) {
    RPCTrafficSourceBuilder sourceBuilder = new RPCTrafficSourceBuilder(namingControl);
