...
public class PromQLExprQueryVisitor extends PromQLParserBaseVisitor<ParseResult> {
    ...
    valueLabel.ifPresent(s -> addValueLabel(metricInfo, s, isMultiIntValues));

    metricInfo.getLabels().add(new LabelValuePair(LabelName.LAYER.getLabel(), layer.name()));

    switch (scope) {
      case Service:
        buildServiceMetricInfo(metricInfo, sysLabelMap, topNEntityName, recordName);
        break;
      case ServiceInstance:
        buildServiceInstanceMetricInfo(metricInfo, sysLabelMap, topNEntityName, recordName);
        break;
      case Endpoint:
        buildEndpointMetricInfo(metricInfo, sysLabelMap, topNEntityName, recordName);
        break;
    }
  ...
  private void addValueLabel(MetricInfo metricInfo, String labelValue, boolean isMultiIntValues) {
    if (isMultiIntValues) {
      metricInfo.getLabels().add(new LabelValuePair(LabelName.LABELS.getLabel(), labelValue));
    } else {
      DataLabel dataLabel = new DataLabel();
      dataLabel.put(labelValue);
  ...
  private void buildServiceMetricInfo(MetricInfo metricInfo,
      Map<LabelName, String> sysLabelMap,
      Optional<String> topNEntityName,
  ...
  private void buildServiceInstanceMetricInfo(MetricInfo metricInfo,
      Map<LabelName, String> sysLabelMap,
      Optional<String> topNEntityName,
      Optional<String> recordName) throws IllegalExpressionException {
  ...
  private void buildEndpointMetricInfo(MetricInfo metricInfo,
      Map<LabelName, String> sysLabelMap,
      Optional<String> topNEntityName,
      Optional<String> recordName) throws IllegalExpressionException {
