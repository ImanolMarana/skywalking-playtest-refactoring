...
public class DispatcherManager implements DispatcherDetectorListener {
    ...
    public void addIfAsSourceDispatcher(Class aClass) throws IllegalAccessException, InstantiationException {
      if (!isSourceDispatcher(aClass)) {
        return;
      }

      Type[] genericInterfaces = aClass.getGenericInterfaces();
      for (Type genericInterface : genericInterfaces) {
        addIfMatchSourceDispatcher(aClass, genericInterface);
      }
    }

    private boolean isSourceDispatcher(Class aClass) {
      return !aClass.isInterface() && !Modifier.isAbstract(aClass.getModifiers()) && SourceDispatcher.class.isAssignableFrom(aClass);
    }

    private void addIfMatchSourceDispatcher(Class aClass, Type genericInterface)
