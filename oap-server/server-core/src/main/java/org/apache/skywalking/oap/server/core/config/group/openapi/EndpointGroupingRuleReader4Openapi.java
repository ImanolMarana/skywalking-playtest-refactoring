...
public class EndpointGroupingRuleReader4Openapi {
    ...
    serviceOpenapiDefMap.forEach((serviceName, openapiDefs) -> {
      openapiDefs.forEach(openapiData -> processOpenapiData(endpointGroupingRule, serviceName, openapiData));
    });
    endpointGroupingRule.sortRulesAll();
    return endpointGroupingRule;
  }

  private void processOpenapiData(EndpointGroupingRule4Openapi endpointGroupingRule, String serviceName, Map openapiData) {
    ...
    if (paths != null) {
      paths.forEach((pathString, pathItem) -> {
        pathItem.keySet().forEach(key -> processPathItem(endpointGroupingRule, serviceName, openapiData, pathString, key));
      });
    }
  }

  private void processPathItem(EndpointGroupingRule4Openapi endpointGroupingRule, String serviceName, Map openapiData, String pathString, String key) {
