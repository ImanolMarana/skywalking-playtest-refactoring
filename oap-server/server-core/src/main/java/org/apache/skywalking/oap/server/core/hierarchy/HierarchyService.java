        ...
        if (allServices.size() > 1) {
            for (int i = 0; i < allServices.size(); i++) {
                for (int j = i + 1; j < allServices.size(); j++) {
                    processServicePair(allServices.get(i), allServices.get(j));
                }
            }
        }
    }
    
    private void processServicePair(Service service1, Service service2) {
        String service1Layer = service1.getLayers().iterator().next();
        String service2Layer = service2.getLayers().iterator().next();
    
        Map<String, HierarchyDefinitionService.MatchingRule> lowerLayers = getHierarchyDefinition().get(service1Layer);
        Map<String, HierarchyDefinitionService.MatchingRule> comparedLowerLayers = getHierarchyDefinition().get(service2Layer);
    
        if (lowerLayers != null && lowerLayers.get(service2Layer) != null) {
            evaluateAndMatchHierarchy(service1, service2, service1Layer, service2Layer, lowerLayers.get(service2Layer));
        } else if (comparedLowerLayers != null && comparedLowerLayers.get(service1Layer) != null) {
            evaluateAndMatchHierarchy(service2, service1, service2Layer, service1Layer, comparedLowerLayers.get(service1Layer));
        }
    }
    
    private void evaluateAndMatchHierarchy(Service upperService, Service lowerService, String upperLayer, String lowerLayer, HierarchyDefinitionService.MatchingRule matchingRule) {
        try {
            if (matchingRule.getClosure().call(upperService, lowerService)) {
                autoMatchingServiceRelation(upperService.getName(), Layer.nameOf(upperLayer),
                        lowerService.getName(),
                        Layer.nameOf(lowerLayer)
                );
            }
        } catch (Throwable e) {
            log.error(
