...
public class EBPFProfilingMutationService implements Service {
    ...
    private String checkCreateRequest(EBPFProfilingTaskFixedTimeCreationRequest request) throws IOException {
        String err = requiredNotEmpty(null, "service", request.getServiceId());

        // the request label must be legal
        if (err == null && CollectionUtils.isNotEmpty(request.getProcessLabels())) {
            err = validateProcessLabels(request);
        }
        if (err != null) {
            return err;
        }
        ...
        return validateTaskTimeRange(request);
    }
    
    private String validateProcessLabels(EBPFProfilingTaskFixedTimeCreationRequest request) throws IOException {
        ...
        if (notExistLabels.size() > 0) {
            return String.format("The service doesn't have processes with label(s) %s.", Joiner.on(", ").join(notExistLabels));
        } else {
            ...
            if (labelJson.length() > EBPFProfilingTaskRecord.PROCESS_LABELS_JSON_MAX_LENGTH) {
                return String.format("The labels length is bigger than %d, please reduce the labels count",
    ...
    private String validateTaskTimeRange(EBPFProfilingTaskFixedTimeCreationRequest request) throws IOException {
