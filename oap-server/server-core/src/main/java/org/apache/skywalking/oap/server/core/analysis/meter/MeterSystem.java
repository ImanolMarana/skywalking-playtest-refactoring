...
public class MeterSystem implements Service {
    ...
    public synchronized <T> void create(String metricsName,
        ...
        validateMeterFunction(metricsName, functionName, dataType, meterFunction);
        ...
        if (isMetricAlreadyDefined(metricsName, type, parentClass, className)) {
            return;
        }

        CtClass metricsClass = createMetricsClass(parentClass, className);

        createNewConstructor(metricsClass, className);

        generateCreateNewMethod(metricsClass, className, type, metricsName);

        createAndRegisterMetric(metricsName, type, dataType, metricsClass);
    }

    private <T> void validateMeterFunction(String metricsName, String functionName, Class<T> dataType, Class<? extends AcceptableValue> meterFunction) {
        if (meterFunction == null) {
            throw new IllegalArgumentException("Function " + functionName + " can't be found.");
        }
    ...
    private boolean isMetricAlreadyDefined(String metricsName, ScopeType type, CtClass parentClass, String className) {
        try {
    ...
    private CtClass createMetricsClass(CtClass parentClass, String className) {
        CtClass metricsClass = classPool.makeClass(METER_CLASS_PACKAGE + className, parentClass);
        return metricsClass;
    }

    private void createNewConstructor(CtClass metricsClass, String className) {
        try {
    ...
    private void generateCreateNewMethod(CtClass metricsClass, String className, ScopeType type, String metricsName) {
        try {
    ...
    private void createAndRegisterMetric(String metricsName, ScopeType type, Class dataType, CtClass metricsClass) {
        Class targetClass;
        try {
            ...
            log.error("Can't compile/load/init " + metricsClass.getName() + ".", e);
            throw new UnexpectedException(e.getMessage(), e);
        }
    }
