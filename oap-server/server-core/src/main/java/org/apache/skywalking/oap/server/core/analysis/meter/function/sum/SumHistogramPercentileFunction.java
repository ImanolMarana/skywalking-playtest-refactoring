...
public abstract class SumHistogramPercentileFunction extends Meter implements AcceptableValue<PercentileArgument>, LabeledValueHolder {
    ...
    public void calculate() {
      if (!isCalculated) {
        Map<DataLabel, DataTable> datasetPerLabel = summation.keys().stream()
            .map(this::convertToTuple)
            .collect(groupingBy(Tuple2::_1, mapping(Tuple2::_2, Collector.of(
                DataTable::new,
                this::accumulateValues,
                DataTable::append
            ))));

        datasetPerLabel.forEach(this::calculatePercentiles);
      }
    }

    private Tuple2<DataLabel, String> convertToTuple(String key) {
    ...
    private void accumulateValues(DataTable dt, String key) {
      String v;
      if (key.contains(":")) {
        int index = key.lastIndexOf(":");
    ...
    private void calculatePercentiles(DataLabel labels, DataTable subDataset) {
