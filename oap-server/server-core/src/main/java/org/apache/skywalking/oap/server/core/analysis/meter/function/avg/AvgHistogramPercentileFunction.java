...
public abstract class AvgHistogramPercentileFunction extends Meter implements AcceptableValue<PercentileArgument>, LabeledValueHolder {
  ...
  private void calculatePercentiles() {
    ...
    .forEach((labels, subDataset) -> calculatePercentileForSubDataset(labels, subDataset));
  }
  
  private void calculatePercentileForSubDataset(final DataLabel labels, final DataTable subDataset) {
    long total = subDataset.sumOfValues();
    int[] roofs = calculateRoofs(total);
    ...
    for (String key : sortedKeys) {
      ...
      count += value;
      loopIndex = calculatePercentileValues(labels, subDataset, roofs, count, loopIndex, key);
    }
  }
  
  private int[] calculateRoofs(final long total) {
  ...
  private int calculatePercentileValues(final DataLabel labels, final DataTable subDataset, final int[] roofs, 
                                       int count, int loopIndex, final String key) {
