...
public class TraceQueryService implements Service {
    ...
    // sort by start time
    events.sort(Comparator.comparingLong(SpanAttachedEventRecord::getStartTimeSecond)
      .thenComparingLong(SpanAttachedEventRecord::getStartTimeNanos));
    ...
    for (SpanAttachedEventRecord record : events) {
      ...
      spanMatcher.computeIfAbsent(spanMatcherKey, k -> findOrCreateSpan(spans, record, event))
        .ifPresent(span -> span.getAttachedEvents().add(parseEvent(event)));
    }
  }

  private Span findOrCreateSpan(List<Span> spans, SpanAttachedEventRecord record, SpanAttachedEvent event) {
    final int eventSpanId = Integer.parseInt(record.getTraceSpanId());
    Span span = spans.stream()
      .filter(s -> Objects.equals(s.getSegmentId(), record.getTraceSegmentId()) &&
        (s.getSpanId() == eventSpanId))
  ...
  private String getSpanAttachedEventTagValue(List<KeyStringValuePair> values, String tagKey) {
    return values.stream()
      .filter(pair -> Objects.equals(pair.getKey(), tagKey))
  ...
  private org.apache.skywalking.oap.server.core.query.type.SpanAttachedEvent parseEvent(SpanAttachedEvent event) {
    ...
    event.getTagsList().forEach(tag -> result.getTags().add(new KeyValue(tag.getKey(), tag.getValue())));
    event.getSummaryList()
      .forEach(pair -> result.getSummary().add(new KeyNumericValue(pair.getKey(), pair.getValue())));
    return result;
  }
