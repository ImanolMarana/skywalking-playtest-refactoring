...
public class SpanForward implements SpanForwardService {
    ...
    getSampledTraces(spanList).forEach(span -> sendSpan(span));
  }

  private void sendSpan(Span span) {
    ZipkinSpan zipkinSpan = buildZipkinSpan(span);
    long minuteTimeBucket = TimeBucket.getMinuteTimeBucket(span.timestampAsLong() / 1000);

    processSpanTagsAndAnnotations(zipkinSpan, minuteTimeBucket);
  ...
  private ZipkinSpan buildZipkinSpan(Span span) {
    ...
    zipkinSpan.setTimeBucket(TimeBucket.getRecordTimeBucket(timestampMillis));
    return zipkinSpan;
  }

  private void processSpanTagsAndAnnotations(ZipkinSpan zipkinSpan, long minuteTimeBucket) {
    if (!zipkinSpan.getTags().isEmpty() || !zipkinSpan.getAnnotations().isEmpty()) {
      ...
      for (Annotation annotation : zipkinSpan.getAnnotations()) {
        ...
        zipkinSpan.getQuery().add(annotation.value());
      }
      zipkinSpan.setAnnotations(annotationsJson);
      for (Map.Entry<String, String> tag : zipkinSpan.getTags().entrySet()) {
        ...
        zipkinSpan.getQuery().add(tag.getKey());
        zipkinSpan.getQuery().add(tagString);
