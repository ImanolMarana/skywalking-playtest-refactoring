...
public class OpenTelemetryMetricRequestProcessor implements Service {
    ...
    private Stream<? extends Metric> adaptMetrics(
        ...
        if (metric.hasGauge()) {
          return adaptGaugeMetrics(nodeLabels, metric);
        }
        if (metric.hasSum()) {
          return adaptSumMetrics(nodeLabels, metric);
        }
        if (metric.hasHistogram()) {
          return adaptHistogramMetrics(nodeLabels, metric);
        }
        if (metric.hasExponentialHistogram()) {
          return adaptExponentialHistogramMetrics(nodeLabels, metric);
        }
        if (metric.hasSummary()) {
          return adaptSummaryMetrics(nodeLabels, metric);
        }
        throw new UnsupportedOperationException("Unsupported type");
      }
    
      private Stream<? extends Metric> adaptGaugeMetrics(
          final Map<String, String> nodeLabels,
          final io.opentelemetry.proto.metrics.v1.Metric metric) {
      ...
      private Stream<? extends Metric> adaptSumMetrics(
          final Map<String, String> nodeLabels,
          final io.opentelemetry.proto.metrics.v1.Metric metric) {
      ...
      private Stream<? extends Metric> adaptHistogramMetrics(
          final Map<String, String> nodeLabels,
          final io.opentelemetry.proto.metrics.v1.Metric metric) {
      ...
      private Stream<? extends Metric> adaptExponentialHistogramMetrics(
          final Map<String, String> nodeLabels,
          final io.opentelemetry.proto.metrics.v1.Metric metric) {
      ...
      private Stream<? extends Metric> adaptSummaryMetrics(
          final Map<String, String> nodeLabels,
          final io.opentelemetry.proto.metrics.v1.Metric metric) {
