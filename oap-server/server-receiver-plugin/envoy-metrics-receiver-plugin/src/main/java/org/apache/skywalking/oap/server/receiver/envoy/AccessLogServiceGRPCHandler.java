...
public class AccessLogServiceGRPCHandler extends AccessLogServiceGrpc.AccessLogServiceImplBase {
    ...
    return new StreamObserver<StreamAccessLogsMessage>() {
      ...
      public void onNext(StreamAccessLogsMessage message) {
        ...
        try {
          processAccessLogMessage(message, alwaysAnalyzeIdentity);
        } finally {
          timer.finish();
        }
      }

      private void processAccessLogMessage(StreamAccessLogsMessage message, boolean alwaysAnalyzeIdentity) {
        if (isFirst || alwaysAnalyzeIdentity && message.hasIdentifier()) {
          identifier = message.getIdentifier();
          isFirst = false;
          role = identifyRole(identifier);
        }
        ...
        if (LOGGER.isDebugEnabled()) {
          LOGGER.debug(
              ...
              identifier.getNode().getId(),
              role,
              logCase,
              message);
        }

        ServiceMeshMetrics.Builder sourceResult = processLogEntries(message, logCase);
        sourceDispatcherCounter.inc(
            sourceResult.getHttpMetrics().getMetricsCount() + sourceResult.getTcpMetrics()
                .getMetricsCount());
        TelemetryDataDispatcher.process(sourceResult.build());

      }

      private ServiceMeshMetrics.Builder processLogEntries(StreamAccessLogsMessage message,
          StreamAccessLogsMessage.LogEntriesCase logCase) {
        final ServiceMeshMetrics.Builder sourceResult = ServiceMeshMetrics.newBuilder();
        switch (logCase) {
          case HTTP_LOGS:
            processHttpLogs(sourceResult, message);
            break;
          case TCP_LOGS:
            processTcpLogs(sourceResult, message);
            break;
          default: // Ignored
        }
        return sourceResult;
      ...
      private void processTcpLogs(ServiceMeshMetrics.Builder sourceResult,
          StreamAccessLogsMessage message) {
      ...
      private void processHttpLogs(ServiceMeshMetrics.Builder sourceResult,
          StreamAccessLogsMessage message) {
      ...
      private Role identifyRole(StreamAccessLogsMessage.Identifier identifier) {
        Role currentRole = Role.NONE;
        for (ALSHTTPAnalysis analysis : envoyHTTPAnalysisList) {
          currentRole = analysis.identify(identifier, currentRole);
        }
        return currentRole;
      }
