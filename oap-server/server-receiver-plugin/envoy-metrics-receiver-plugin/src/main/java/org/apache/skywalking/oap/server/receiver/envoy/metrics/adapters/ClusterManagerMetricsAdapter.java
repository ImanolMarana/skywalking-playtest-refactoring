...
public class ClusterManagerMetricsAdapter {
    ...
    protected ServiceMetaInfo buildUpstreamServiceMetaInfo(final Metrics.MetricFamily metricFamily) throws Exception {
        ...
        if (metricsName.startsWith("cluster.outbound")) {
            extractOutboundServiceInfo(metricsName, serviceName, version, ns);
        } else if (metricsName.startsWith("cluster.inbound")) {
            extractInboundServiceInfo(metricsName, serviceName);
        } else if (StringUtils.countMatches(metricsName, ".") == 2) {
            serviceName = metricsName.split("\\.")[1];
        }
    ...
    private void extractOutboundServiceInfo(String metricsName, String serviceName, String version, String ns) {
    ...
    private void extractInboundServiceInfo(String metricsName, String serviceName) {
        String[] splitArrBound = metricsName.split("\\|");
