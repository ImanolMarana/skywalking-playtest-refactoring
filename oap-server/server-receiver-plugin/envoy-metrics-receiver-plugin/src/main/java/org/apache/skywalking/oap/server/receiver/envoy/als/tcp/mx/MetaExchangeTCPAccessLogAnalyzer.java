...
public class MetaExchangeTCPAccessLogAnalyzer extends AbstractTCPAccessLogAnalyzer {
    ...
    public Result analysis(
        ...
        return processStateMap(newResult, previousMetrics, stateMap, currSvc, entry, role);
    }

    private Result processStateMap(final Result.Builder newResult,
                                    final TCPServiceMeshMetric.Builder previousMetrics,
        ...
        stateMap.forEach((key, value) -> processState(key, value, currSvc, entry, tcpMetrics, newResult, downstreamExists));
    ...
    private void processState(final String key,
                              final Any value,
                              final ServiceMetaInfo currSvc,
                              final TCPAccessLogEntry entry,
                              final TCPServiceMeshMetric.Builder tcpMetrics,
                              final Result.Builder newResult,
        ...
        switch (key) {
            case UPSTREAM_KEY:
                processUpstream(entry, currSvc, svc, tcpMetrics, newResult);
                break;
            case DOWNSTREAM_KEY:
                processDownstream(entry, currSvc, svc, tcpMetrics, newResult, downstreamExists);
                break;
        }
    }

    private void processUpstream(final TCPAccessLogEntry entry,
                                 final ServiceMetaInfo currSvc,
                                 final ServiceMetaInfo svc,
                                 final TCPServiceMeshMetric.Builder tcpMetrics,
                                 final Result.Builder newResult) {
        if (newResult.hasUpstreamMetrics()) {
            return;
        }
        final TCPServiceMeshMetric.Builder metrics = newAdapter(entry, currSvc, svc).adaptToUpstreamMetrics().setTlsMode(NON_TLS);
        if (log.isDebugEnabled()) {
    ...
    private void processDownstream(final TCPAccessLogEntry entry,
                                   final ServiceMetaInfo currSvc,
                                   final ServiceMetaInfo svc,
                                   final TCPServiceMeshMetric.Builder tcpMetrics,
                                   final Result.Builder newResult,
                                   final AtomicBoolean downstreamExists) {
        if (newResult.hasDownstreamMetrics()) {
            return;
        }
        final TCPServiceMeshMetric.Builder metrics = newAdapter(entry, svc, currSvc).adaptToDownstreamMetrics();
        if (log.isDebugEnabled()) {
