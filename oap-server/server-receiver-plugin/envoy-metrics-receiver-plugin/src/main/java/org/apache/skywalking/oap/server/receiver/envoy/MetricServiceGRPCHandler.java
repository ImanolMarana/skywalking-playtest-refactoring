        ...
        return new StreamObserver<StreamMetricsMessage>() {
            ...
            public void onNext(StreamMetricsMessage message) {
                ...
                if (service != null && StringUtil.isNotEmpty(service.getServiceName()) && StringUtil.isNotEmpty(
                    ...
                    processMetrics(message.getEnvoyMetricsList());
                }
            }

            private void processMetrics(List<Metrics.MetricFamily> metricFamilies) {
                Map<String, List<Metric>> groupingMetrics = new HashMap<>();
                for (final Metrics.MetricFamily metricFamily : metricFamilies) {
                    counter.inc();
                    try (final HistogramMetrics.Timer ignored = histogram.createTimer()) {
                        ...
                        adapter.adapt().forEach(this::groupMetric);
                    }
                }
            ...
            private void groupMetric(Metric metric) {
                metric.getLabels().putIfAbsent("app", service.getServiceName());
                metric.getLabels().putIfAbsent("instance", service.getServiceInstanceName());
