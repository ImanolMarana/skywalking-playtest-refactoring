...
public class ApplicationConfigLoader implements ConfigLoader<ApplicationConfiguration> {
    ...
    try {
        ...
        if (CollectionUtils.isNotEmpty(moduleConfig)) {
            ...
            moduleConfig.forEach(this::loadModuleConfiguration);
        }
...
private void loadModuleConfiguration(String moduleName, Map<String, Object> providerConfig) {
    if (providerConfig.size() > 0) {
        ...
        providerConfig.forEach((providerName, config) -> {
            loadProviderConfiguration(moduleName, moduleConfiguration, providerName, config);
        });
    } else {
        log.warn(
...
private void loadProviderConfiguration(String moduleName, ApplicationConfiguration.ModuleConfiguration moduleConfiguration, String providerName, Object config) {
    log.info(
    ...
    if (propertiesConfig != null) {
        propertiesConfig.forEach((propertyName, propertyValue) -> {
            loadProperty(properties, providerName, propertyName, propertyValue);
        });
    }
...
private void loadProperty(Properties properties, String providerName, String propertyName, Object propertyValue) {
