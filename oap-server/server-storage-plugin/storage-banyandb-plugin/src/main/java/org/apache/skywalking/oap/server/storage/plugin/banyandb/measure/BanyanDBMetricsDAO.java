...
public class BanyanDBMetricsDAO extends AbstractBanyanDBDAO implements IMetricsDAO {
    ...
    private AnalyticalResult analyze(List<StorageID.Fragment> fragments, String tsCol, Map<String, List<String>> seriesIDColumns) {
        ...
        for (StorageID.Fragment f : fragments) {
            if (!analyzeFragment(f, tsCol, seriesIDColumns, result)) {
                return result;
            }
        }
        result.success = true;
    ...
    private boolean analyzeFragment(StorageID.Fragment fragment, String tsCol, Map<String, List<String>> seriesIDColumns, AnalyticalResult result) {
        Optional<String[]> cols = fragment.getName();
        if (!cols.isPresent()) {
            log.error("fragment [{}] in id [{}] doesn't contains cols", fragment, fragments);
            return false;
        }
        ...
        for (String col : cols.get()) {
            if (tsCol.equals(col)) {
                handleTimestampColumn(fragment, result);
            } else if (seriesIDColumns.containsKey(col)) {
                handleSeriesIDColumn(fragment, col, seriesIDColumns);
            } else {
                log.error("col [{}] in fragment [{}] id [{}] is not ts or seriesID", col, fragment, fragments);
                return false;
            }
        }
        return true;
    ...
    private void handleTimestampColumn(StorageID.Fragment fragment, AnalyticalResult result) {
        long timeBucket = (long) fragment.getValue();
    ...
    private void handleSeriesIDColumn(StorageID.Fragment fragment, String col, Map<String, List<String>> seriesIDColumns) {
        Preconditions.checkState(fragment.getType().equals(String.class));
        seriesIDColumns.get(col).add((String) fragment.getValue());
    }
