...
public class BatchProcessEsDAO extends EsDAO implements IBatchDAO {
    ...
    public CompletableFuture<Void> flush(List<PrepareRequest> prepareRequests) {
        ...
        return CompletableFuture.allOf(prepareRequests.stream().map(this::processRequest).toArray(CompletableFuture[]::new));
    }

    private CompletableFuture<Void> processRequest(PrepareRequest prepareRequest) {
        if (prepareRequest instanceof InsertRequest) {
            return processInsertRequest((InsertRequest) prepareRequest);
        } else {
            return processUpdateRequest((UpdateRequestWrapper) prepareRequest);
        }
    }

    private CompletableFuture<Void> processInsertRequest(InsertRequest insertRequest) {
        return bulkProcessor.add(((IndexRequestWrapper) insertRequest).getRequest())
                           .whenComplete((v, throwable) -> {
                               if (throwable == null) {
                                   // Insert completed
                                   ((IndexRequestWrapper) insertRequest).onInsertCompleted();
                               }
                           });
    }

    private CompletableFuture<Void> processUpdateRequest(UpdateRequestWrapper updateRequest) {
        return bulkProcessor.add(updateRequest.getRequest())
                           .whenComplete((v, throwable) -> {
                               if (throwable != null) {
                                   // Update failure
                                   updateRequest.onUpdateFailure();
                               }
                           });
    }
