...
public class ESEventQueryDAO extends EsDAO implements IEventQueryDAO {
    ...
    private void buildMustQueryListByCondition(final EventQueryCondition condition,
        ...
        addUuidQuery(condition, query);
        addSourceQuery(condition, query);
        addNameQuery(condition, query);
        addTypeQuery(condition, query);
        addTimeRangeQuery(condition, query);
        addLayerQuery(condition, query);
    }

    private void addUuidQuery(EventQueryCondition condition, BoolQueryBuilder query) {
        if (!isNullOrEmpty(condition.getUuid())) {
            query.must(Query.term(Event.UUID, condition.getUuid()));
        }
    }

    private void addSourceQuery(EventQueryCondition condition, BoolQueryBuilder query) {
    ...
    private void addNameQuery(EventQueryCondition condition, BoolQueryBuilder query) {
        if (!isNullOrEmpty(condition.getName())) {
            query.must(Query.term(Event.NAME, condition.getName()));
        }
    }

    private void addTypeQuery(EventQueryCondition condition, BoolQueryBuilder query) {
        if (condition.getType() != null) {
            query.must(Query.term(Event.TYPE, condition.getType().name()));
        }
    }

    private void addTimeRangeQuery(EventQueryCondition condition, BoolQueryBuilder query) {
    ...
    private void addLayerQuery(EventQueryCondition condition, BoolQueryBuilder query) {
