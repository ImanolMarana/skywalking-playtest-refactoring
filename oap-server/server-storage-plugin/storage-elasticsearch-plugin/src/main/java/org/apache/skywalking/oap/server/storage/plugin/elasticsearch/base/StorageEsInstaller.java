...
public class StorageEsInstaller extends ModelInstaller {
    ...
    try {
      createTimeSeriesTableInternal(model, esClient, tableName, settings, mapping);
    } catch (IOException e) {
      throw new StorageException("cannot create " + tableName + " index template", e);
    }
  }

  private void createTimeSeriesTableInternal(Model model, ElasticSearchClient esClient, String tableName,
                                           Map<String, Object> settings, Mappings mapping) throws IOException, StorageException {
    String indexName = TimeSeriesUtils.latestWriteIndexName(model);
    boolean shouldUpdateTemplate = shouldUpdateTemplate(esClient, tableName, settings, mapping);
    ...
    if (esClient.isExistsIndex(indexName)) {
      updateIndex(esClient, tableName, indexName, settings);
    } else {
      createIndex(esClient, indexName);
    }
  }

  private boolean shouldUpdateTemplate(ElasticSearchClient esClient, String tableName, Map<String, Object> settings,
                                    Mappings mapping) throws IOException {
    ...
    return shouldUpdateTemplate;
  }

  private void updateIndex(ElasticSearchClient esClient, String tableName, String indexName,
                           Map<String, Object> settings) throws StorageException, IOException {
  ...
  private void createIndex(ElasticSearchClient esClient, String indexName) throws StorageException {
