...
public class JDBCMetadataQueryDAO implements IMetadataQueryDAO {
    ...
    private void appendProcessWhereQuery(StringBuilder sql, List<Object> condition, String serviceId, String instanceId,
        ...
        if (StringUtil.isNotEmpty(serviceId)) {
            appendCondition(sql, condition, ProcessTraffic.SERVICE_ID, serviceId);
        }
        if (StringUtil.isNotEmpty(instanceId)) {
            appendCondition(sql, condition, ProcessTraffic.INSTANCE_ID, instanceId);
        }
        if (StringUtil.isNotEmpty(agentId)) {
            appendCondition(sql, condition, ProcessTraffic.AGENT_ID, agentId);
        }
        if (profilingSupportStatus != null) {
            appendCondition(sql, condition, ProcessTraffic.PROFILING_SUPPORT_STATUS, profilingSupportStatus.value());
        }
        if (lastPingStartTimeBucket > 0) {
            appendCondition(sql, condition, ProcessTraffic.LAST_PING_TIME_BUCKET, ">=", lastPingStartTimeBucket);
        }
        if (!includeVirtual) {
            appendCondition(sql, condition, ProcessTraffic.DETECT_TYPE, "!=", ProcessDetectType.VIRTUAL.value());
        }
    }

    private void appendCondition(StringBuilder sql, List<Object> condition, String field, Object value) {
    ...
    private void appendCondition(StringBuilder sql, List<Object> condition, String field, String operator, Object value) {
        if (!condition.isEmpty()) {
            sql.append(" and ");
        }
