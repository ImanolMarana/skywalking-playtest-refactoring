...
public class JDBCTraceQueryDAO implements ITraceQueryDAO {
    ...
    for (String table : tables) {
      ...
      appendJoinQuery(sql, table, tags);
      appendWhereQuery(sql, parameters, table, startSecondTB, endSecondTB, minDuration, maxDuration, serviceId,
                       serviceInstanceId, endpointId, traceId, traceState, tags
      );
      appendOrderByQuery(sql, queryOrder);
      buildLimit(sql, from, limit);

      executeQuery(sql, parameters, traces);
    }

    return new TraceBrief(traces); // TODO: sort,
  }

  private void executeQuery(StringBuilder sql, List<Object> parameters, List<BasicTrace> traces) throws SQLException {
  ...
  private void appendOrderByQuery(StringBuilder sql, QueryOrder queryOrder) {
    switch (queryOrder) {
      case BY_START_TIME:
  ...
  private void appendWhereQuery(StringBuilder sql, List<Object> parameters, String table, long startSecondTB,
                                long endSecondTB, long minDuration, long maxDuration, String serviceId,
                                String serviceInstanceId, String endpointId, String traceId, TraceState traceState,
                                List<Tag> tags) {
    sql.append(" where ");
    ...
    if (CollectionUtils.isNotEmpty(tags)) {
      for (int i = 0; i < tags.size(); i++) {
        String tagTable = "tagTable" + i;
        sql.append(" and ").append(tagTable).append(".");
  ...
  private void appendJoinQuery(StringBuilder sql, String table, List<Tag> tags) {
    /*
    ...
    if (!CollectionUtils.isEmpty(tags)) {
      for (int i = 0; i < tags.size(); i++) {
        ...
        sql.append("tagTable").append(i);
        sql.append(" on ").append(table).append(".").append(JDBCTableInstaller.ID_COLUMN).append(" = ");
        sql.append("tagTable").append(i).append(".").append(JDBCTableInstaller.ID_COLUMN);
      }
    }
  }
